import React, { useState, useEffect, useCallback, Suspense, lazy } from 'react';
import ErrorBoundary from './ErrorBoundary';

// three.js + the physics canvas are ~430 kB gzipped. Loading them lazily lets the
// hero copy paint first, which is what Google measures for LCP.
const Physics3DHeroCanvas = lazy(() => import('./Physics3DHeroCanvas'));

// On desktop devices, immediately trigger background fetch of the 3D bundle
if (typeof window !== 'undefined' && window.matchMedia && !window.matchMedia('(max-width: 820px)').matches) {
  import('./Physics3DHeroCanvas');
}

const BACKDROP =
  'radial-gradient(ellipse at 62% 55%, var(--clay-700) 0%, var(--clay-800) 55%, var(--clay-900) 100%)';

/**
 * Diagnostics live at the TOP of the hero, horizontally centred.
 * Turn it off with ?diag=0 once the hero is confirmed working.
 */
function DiagPanel({ stats, note }) {
  const rows = Object.entries(stats);
  const bad = rows.some(([, v]) => /FAIL|UNMOUNT|unavailable/i.test(String(v)));

  return (
    <div
      style={{
        position: 'absolute', top: '14px', left: '50%', transform: 'translateX(-50%)',
        zIndex: 5, maxWidth: 'min(560px, calc(100% - 28px))',
        padding: '12px 16px', borderRadius: '8px',
        background: 'rgba(4, 10, 24, 0.86)',
        border: `1px solid ${bad ? 'rgba(220,60,60,0.7)' : 'color-mix(in srgb, var(--on-accent) 22%, transparent)'}`,
        color: '#E8EDF5', fontFamily: 'var(--font-mono)', fontSize: '0.7rem',
        lineHeight: 1.7, pointerEvents: 'none', backdropFilter: 'blur(6px)'
      }}
    >
      <strong style={{ display: 'block', marginBottom: '4px', letterSpacing: '0.1em' }}>
        HERO 3D DIAGNOSTIC {bad ? '— PROBLEM DETECTED' : ''}
      </strong>
      {note && <div style={{ marginBottom: '4px', color: '#FFB4B4' }}>{note}</div>}
      {rows.length === 0 && <div>waiting for the 3D chunk…</div>}
      {rows.map(([k, v]) => (
        <div key={k}>
          <span style={{ opacity: 0.6 }}>{k}:</span>{' '}
          <span style={{ color: /FAIL|UNMOUNT/i.test(String(v)) ? '#FF9A9A' : '#B8F5C8' }}>{v}</span>
        </div>
      ))}
      <div style={{ marginTop: '6px', opacity: 0.55 }}>append ?diag=0 to hide · ?ao=0 disables AO</div>
    </div>
  );
}

function flag(name, fallback) {
  try {
    const v = new URLSearchParams(window.location.search).get(name);
    return v === null ? fallback : v !== '0';
  } catch {
    return fallback;
  }
}

/**
 * Whether this device should get the interactive scene directly.
 * Only falls back for mobile view (<= 820px).
 */
function shouldRender3D() {
  try {
    const q = new URLSearchParams(window.location.search).get('3d');
    if (q === '1') return true;
    if (q === '0') return false;
    // Mobile view fallback
    if (window.matchMedia('(max-width: 820px)').matches) return false;
    return true;
  } catch {
    return true;
  }
}

/**
 * Static stand-in for the 3D bed. Used as mobile view fallback and seamless Suspense placeholder.
 */
function HeroPoster({ onEnable, isMobileFallback = false }) {
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', background: BACKDROP }}>
      <picture>
        <source
          type="image/avif"
          media="(max-width: 820px)"
          srcSet={`${import.meta.env.BASE_URL}hero/products-hero-900.avif`}
        />
        <source
          type="image/webp"
          media="(max-width: 820px)"
          srcSet={`${import.meta.env.BASE_URL}hero/products-hero-900.webp`}
        />
        <source
          type="image/avif"
          srcSet={`${import.meta.env.BASE_URL}hero/products-hero-1600.avif`}
        />
        <source
          type="image/webp"
          srcSet={`${import.meta.env.BASE_URL}hero/products-hero-1600.webp`}
        />
        <img
          src={`${import.meta.env.BASE_URL}hero/products-hero-1600.webp`}
          alt="Graded inert alumina ceramic ball media, from 3 mm through 75 mm"
          width={1600}
          height={900}
          fetchPriority="high"
          decoding="async"
          style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
        />
      </picture>
      <div
        style={{
          position: 'absolute', inset: 0,
          background: 'linear-gradient(to top, color-mix(in srgb, var(--clay-900) 84%, transparent) 0%, color-mix(in srgb, var(--clay-900) 28%, transparent) 55%, color-mix(in srgb, var(--clay-900) 10%, transparent) 100%)'
        }}
      />
      <div
        style={{
          position: 'absolute', left: 0, right: 0, bottom: '32px',
          display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '14px',
          padding: '0 20px', textAlign: 'center'
        }}
      >
        <h1 style={{ color: 'var(--on-accent)', fontWeight: 800, fontSize: 'clamp(1.25rem, 4.5vw, 1.8rem)', lineHeight: 1.25, textShadow: '0 2px 14px rgba(0,0,0,0.5)', margin: 0 }}>
          Inert Alumina Ceramic Balls &amp; Catalyst Bed Support Media
        </h1>
        {isMobileFallback && (
          <>
            <button
              type="button"
              onClick={onEnable}
              aria-label="Load interactive 3D view"
              style={{
                background: 'var(--btn-bg)', color: 'var(--btn-fg)', border: 'none',
                padding: '12px 22px', borderRadius: 'var(--radius-full)',
                fontSize: '0.86rem', fontWeight: 700, cursor: 'pointer', minHeight: '44px',
                boxShadow: '0 10px 26px -12px rgba(0,0,0,0.7)'
              }}
            >
              Load interactive 3D view
            </button>
            <span style={{ color: 'color-mix(in srgb, var(--on-accent) 74%, transparent)', fontSize: '0.78rem' }}>
              Around 2.5 MB — best on Wi-Fi
            </span>
          </>
        )}
      </div>
    </div>
  );
}

export default function FullWidth3DSliderHero() {
  const [ready3d, setReady3d] = useState(false);
  const [timedOut, setTimedOut] = useState(false);
  const [stats, setStats] = useState({});
  const [crashed, setCrashed] = useState(null);
  const [showDiag, setShowDiag] = useState(false);
  const [force3d, setForce3d] = useState(false);
  const [isMobile, setIsMobile] = useState(false);

  useEffect(() => {
    setShowDiag(flag('diag', false));
    const mq = window.matchMedia('(max-width: 820px)');
    setIsMobile(mq.matches);
    const handler = (e) => setIsMobile(e.matches);
    mq.addEventListener('change', handler);
    return () => mq.removeEventListener('change', handler);
  }, []);

  const want3d = force3d || (!isMobile && shouldRender3D());

  // Stable identity: the canvas reports through this from inside useEffect
  const onStat = useCallback((key, value) => {
    setStats((prev) => (prev[key] === value ? prev : { ...prev, [key]: value }));
  }, []);

  useEffect(() => {
    const t = setTimeout(() => setTimedOut(true), 9000);
    return () => clearTimeout(t);
  }, []);

  return (
    <section
      id="home"
      style={{
        position: 'relative',
        width: '100vw',
        height: 'calc(100vh - 76px)',
        minHeight: '660px',
        marginLeft: 'calc(-50vw + 50%)',
        marginRight: 'calc(-50vw + 50%)',
        overflow: 'hidden',
        background: 'var(--clay-900)'
      }}
    >
      {/* Screen-reader and SEO Heading accessible in both 3D and poster modes */}
      <h1
        style={{
          position: 'absolute',
          width: '1px',
          height: '1px',
          padding: 0,
          margin: '-1px',
          overflow: 'hidden',
          clip: 'rect(0, 0, 0, 0)',
          whiteSpace: 'nowrap',
          border: 0
        }}
      >
        Inert Alumina Ceramic Balls &amp; Catalyst Bed Support Media | Platinaa Ceramics
      </h1>

      <div style={{ position: 'absolute', inset: 0, zIndex: 1 }}>
        {want3d ? (
          <ErrorBoundary
            label="hero-3d"
            onError={(e) => setCrashed(e && e.message ? e.message : String(e))}
            fallback={<HeroPoster onEnable={() => window.location.reload()} isMobileFallback={false} />}
          >
            <Suspense fallback={<HeroPoster isMobileFallback={false} />}>
              <Physics3DHeroCanvas onReady={() => setReady3d(true)} onStat={onStat} />
            </Suspense>
          </ErrorBoundary>
        ) : (
          <HeroPoster onEnable={() => setForce3d(true)} isMobileFallback={true} />
        )}
      </div>

      {showDiag && want3d && (
        <DiagPanel
          stats={stats}
          note={
            crashed
              ? `scene threw: ${crashed}`
              : timedOut && !ready3d
                ? 'No frame painted within 9s.'
                : null
          }
        />
      )}
    </section>
  );
}
